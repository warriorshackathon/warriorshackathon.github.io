...
function startSession() {
  ...
  if (teacherImg) {
    firebase.database().ref('123').set({teacherbase64: teacherImg});
    firebase.database().ref('123/studentList').on("child_added", function(snapshot, prevChildKey) {
        $('#teacher-review').show();
        $('#teacher-main').hide();
...
function endStream() {
  $('#streamBtn').hide();
  firebase.database().ref('123/stream').remove();
}

function stream() {
  imgsrc = $('.modal-body')[lessonCount-1].innerHTML;
  name = $('.modal-title')[lessonCount-1].innerHTML;
  firebase.database().ref('123/stream').set({imgsrc: imgsrc, name: name});
  $('#streamBtn').show();
}
